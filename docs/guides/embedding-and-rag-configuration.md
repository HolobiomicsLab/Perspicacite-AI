...
**Config file:** `config/embeddings/openai-large.yml`
...
uv run perspicacite -c config/embeddings/openai-large.yml serve
```
...
**Config file:** `config/embeddings/pubmedbert.yml`
...
```bash
uv run perspicacite -c config/embeddings/pubmedbert.yml serve
...
**Config file:** `config/embeddings/bge-m3.yml`
...
TRANSFORMERS_OFFLINE=1 HF_DATASETS_OFFLINE=1 \
    uv run perspicacite -c config/embeddings/specter2.yml serve &
...
See `config/embeddings/qwen3-14b.yml` for a complete example.
...
# Edit config/embeddings/pubmedbert.yml: llm.default_provider = "ollama", default_model = "qwen3:14b"
```
...
OPENAI_API_KEY=$OPENAI_API_KEY ANTHROPIC_API_KEY=$ANTHROPIC_API_KEY \
    uv run perspicacite -c config/embeddings/openai-large.yml serve
# Edit config/embeddings/openai-large.yml: 
